import { e as createAstro, c as createComponent, r as renderComponent, b as renderHead, a as renderTemplate, d as addAttribute } from './astro/server.d7beac4.js';
import 'piccolore';
import { $ as $$BaseHead, a as $$Header, b as $$Footer } from './Header.d7beac4.js';
import { g as generatePostSlug, S as SITE_DESCRIPTION, a as SITE_TITLE } from './index.d7beac42.js';
...
const $$MicroblogLayout = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`<html lang="en" class="scroll-smooth"> <head>${renderComponent($$result, "BaseHead", $$BaseHead, { "title": `${SITE_TITLE} \xB7 ${title}`, "description": SITE_DESCRIPTION, "speculationUrls": speculationUrls, "speculationEagerness": speculationEagerness })}${renderHead()}</head> <body class="min-h-screen flex flex-col"> ${renderComponent($$result, "Header", $$Header, {})} <main class="flex-1 w-full max-w-285 mx-auto px-4 py-12"> <section> <nav class="mb-6 flex gap-4 text-sm"> <a href="/microblog/" class="text-blue-600 dark:text-blue-400 hover:underline">Home</a> <a href="/microblog/latest/" class="text-blue-600 dark:text-blue-400 hover:underline">Latest</a> <a href="/microblog/today/" class="text-blue-600 dark:text-blue-400 hover:underline">Today</a> <a href="/microblog/all/" class="text-blue-600 dark:text-blue-400 hover:underline">All Microblog</a> </nav> ${posts.length === 0 ? renderTemplate`<p class="text-gray-600 dark:text-gray-300">暂无内容。</p>` : renderTemplate`<ul class="list-none m-0 p-0 space-y-4"> ${posts.map((post) => {
